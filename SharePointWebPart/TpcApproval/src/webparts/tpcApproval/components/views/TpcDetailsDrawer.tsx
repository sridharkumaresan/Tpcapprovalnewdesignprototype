...
import { makeStyles, shorthands, tokens, Button, Spinner, OverlayDrawer, DrawerBody, DrawerHeader, DrawerHeaderTitle } from '@fluentui/react-components';
...
export const TpcDetailsDrawer: React.FunctionComponent<ITpcDetailsViewProps> = ({ requestId, onBack }) => {
  ...
  if (!request) {
    return (
      <OverlayDrawer position="end" open={true} onOpenChange={(e, data) => !data.open && onBack()}>
        <DrawerHeader>
          <DrawerHeaderTitle
            action={
              <Button appearance="subtle" aria-label="Close" icon={<DismissCircle24Regular />} onClick={onBack} />
            }
          >
            Request Not Found
          </DrawerHeaderTitle>
        ...
        <DrawerBody>
          <h2>The request could not be found.</h2>
        </DrawerBody>
      </OverlayDrawer>
    );
  }
  ...
  return (
    <OverlayDrawer 
      position="end" 
      open={true} 
      onOpenChange={(e, data) => !data.open && onBack()}
